...
* ssl_cert - default: {} - настройки ssl  
...
* cgi_modules - default: [], - модули расширения сервера обработчиками запросов  
...
* extra_response_types - default: {'response'}, - Доп.инициализация разных объектов класс Response  
...
Если передан параметр path, и модуль не имеет атрибута Path, то будет создан атрибут Path со значением, переданным в path.  
...
Модуль выбирается, если запрос содержит заголовок Host или слово "any", имеются в атрибуте модуля Hosts, и имеет самое длинное поле Path, среди всех модулей, у которых Path является началом запрошенной url.  
