...
Метод add_middleware добавляет target в список предобработчиков, которые будут вызваны между парсингом данных из сокета и вызовом обработчика запроса.  
Предобработчик - вызываемый объект (callable), которому будут переданы два параметра: запрос и обработчик запроса.  
Все предобработчики будут вызваны в той же последовательности, что и были регестрированы.  
